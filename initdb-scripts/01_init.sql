...
CREATE TABLE IF NOT EXISTS FAMILIES (
    ...
    CONSTRAINT fk_family_member FOREIGN KEY(family_member) REFERENCES FAMILY_MEMBERS(id),
...
CREATE TABLE IF NOT EXISTS GR_AT (
    group_id    SMALLINT NOT NULL,
    attendee_id    SMALLINT NOT NULL,
    CONSTRAINT fk_group_id FOREIGN KEY(group_id) REFERENCES GROUPS(id),
    CONSTRAINT fk_attendee_id FOREIGN KEY(attendee_id) REFERENCES ATENDEES(id),
...
-- Views
-- -------------------- 
CREATE OR REPLACE VIEW view_family AS SELECT 0 as dummy, pid_parent, family_member, pid_child from FAMILIES;
CREATE OR REPLACE VIEW view_plp_roles AS SELECT 0 as dummy, pid, rid from PEOPLE_ROLES;
CREATE OR REPLACE VIEW view_atendees AS SELECT 0 as dummy, attendee_id, group_id from GR_AT;
...
CREATE OR REPLACE VIEW select_atendees AS SELECT c.name as category, g.id as group_id, a.no_atendees, a.event_id
    from ATENDEES a
    INNER JOIN GR_AT ga ON ga.attendee_id = a.id
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO FAMILIES(pid_parent, family_member, pid_child) 
        VALUES(NEW.pid_parent, NEW.family_member, NEW.pid_child);
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO GR_AT(group_id, attendee_id) 
        VALUES(NEW.group_id, NEW.attendee_id);
    ELSIF TG_OP = 'DELETE' THEN
        DELETE FROM GR_AT
        WHERE attendee_id = OLD.attendee_id AND group_id = OLD.group_id;
    END IF;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
